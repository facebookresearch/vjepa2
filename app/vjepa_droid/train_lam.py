...
from app.vjepa_droid.utils_lam import init_opt, init_video_model, load_checkpoint, load_pretrained
from src.models.inverse_dynamics import InverseDynamicsModel
...
# --- Utility function to encode a sequence of frames with the encoder ---
def encode_sequence_with_encoder(clips, encoder, device):
    """
    Encode a sequence of clips using the encoder.
    clips: [B, C, T, H, W]
    Returns: [B, T, embed_dim]
    """
    clips_seq = clips.permute(0, 2, 1, 3, 4)  # [B, T, C, H, W]
    encoded_seq = []
    for t in range(clips_seq.shape[1]):
        frame = clips_seq[:, t].to(device)  # [B, C, H, W]
        with torch.no_grad():
            encoded = encoder(frame)  # [B, embed_dim]
        encoded_seq.append(encoded)
    encoded_seq = torch.stack(encoded_seq, dim=1)  # [B, T, embed_dim]
    return encoded_seq
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def train_step():
                ...
                with torch.no_grad():
                    encoded_seq = encode_sequence_with_encoder(clips, encoder, device)  # [B, T, embed_dim]
                O_t = encoded_seq[:, -2]  # Current state (second last in sequence)
                z_t = inverse_dynamics_model(encoded_seq)  # [B, embed_dim]

                def forward_predictions(O_t, z):

                    def _step_predictor(_O_t, _z, _a, _s, _e):
                        # Passing both O_t and z_t to predictor
                        _out = predictor(_O_t, _z, _a, _s, _e)
                        if normalize_reps:
                            _out = F.layer_norm(_out, (_out.size(-1),))
                        return _out
                    ...
                    z_tf = _step_predictor(_O_t, _z, _a, _s, _e)
                    ...
                    for n in range(1, auto_steps):
                        ...
                        _z_nxt = _step_predictor(_O_t, _z, _a, _s, _e)[:, -tokens_per_frame:]
                ...
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    ...
                    # --- Pass O_t and z_t (latent action) to predictor ---
                    z_tf, z_ar = forward_predictions(O_t, z_t)
                    # ---------------------------------------------------------
