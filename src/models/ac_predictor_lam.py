...
class VisionTransformerPredictorAC(nn.Module):
    ...
    def forward(self, O_t_seq, z_t_seq, actions, states, extrinsics=None):
        """
        :param O_t_seq: encoder outputs for context, shape [B, T, embed_dim]
        :param z_t_seq: latent actions for context, shape [B, T, embed_dim]
        :param actions: action tokens
        :param states: state tokens
        :param extrinsics: extrinsics tokens (optional)
        """
        # Concatenate O_t and z_t along last dim for each time step
        x = torch.cat([O_t_seq, z_t_seq], dim=-1)  # [B, T, embed_dim*2]
        x = self.predictor_embed(x)                # [B, T, predictor_embed_dim]
        B, T, D = x.size()
