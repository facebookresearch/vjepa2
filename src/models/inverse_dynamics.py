...
class InverseDynamicsModel(nn.Module):
    ...
    Given a sequence of encoded observations (O_{t-h}, ..., O_t, O_{t+1}),
    predicts the latent action z_t that caused the transition from O_t to O_{t+1}.
    Output z_t has the same shape as O_t. - FOR OUR CONVENIENCE AT THE MOMENT
    """
    def __init__(
        self,
        embed_dim=768,
    ...
    def forward(self, encoded_sequence):
        """
        encoded_sequence: Tensor of shape [batch, seq_len, embed_dim]
        Returns: z_t of shape [batch, embed_dim]
        """
        pooled = self.pooler(encoded_sequence)  # [batch, 1, embed_dim]
        pooled = pooled.squeeze(1)              # [batch, embed_dim]
        z_t = self.linear(pooled)               # [batch, embed_dim]
        return z_t
